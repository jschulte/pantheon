...
- Added **Epic Retrospective** workflow (`/epic-retrospective`)
  - Automated artifact-driven retrospective — ingests all build artifacts from a completed epic
  - Clio (Muse of History) analyst agent performs cross-story pattern analysis
  - Produces 5 outputs: retrospective document, playbook proposals, CLAUDE.md patches, Pantheon process suggestions, structured analysis JSON
  - Three actionable output tiers: playbooks (apply in project), CLAUDE.md (apply with high bar), Pantheon suggestions (manual review)
  - Previous retro continuity — tracks action item follow-through across epics
  - Next epic readiness assessment with dependency and technical debt analysis
  - Single user checkpoint (Phase 4 PRESENT) for approving all changes
  - CLAUDE.md patch proposals held to very high bar (majority-of-stories threshold, pointer-to-playbook pattern preferred)
- New workflow version: epic-retrospective `1.0.0`
- New skill: `pantheon-retrospective`
- Totals: 12 `.agent.yaml` files, 18 `workflow.yaml` files

### v1.1.0 (2026-03-12)

- **BREAKING:** Removed `src/adapters/` directory (copilot, opencode, codex, universal)
  - Platform-specific launchers are now auto-generated by BMAD's IDE manager from `.agent.yaml` and `workflow.yaml` files
  - No hand-crafted adapters to maintain or drift-check
- Removed `scripts/detect-adapter-drift.sh` and `npm run check:drift`
- Added 6 standalone `.agent.yaml` files for reviewers/specialists:
...
- Added missing `workflow.yaml` for `ux-audit` and `rally-sync` workflows
- Totals: 12 `.agent.yaml` files, 17 `workflow.yaml` files
- Added structural tests for `.agent.yaml` required fields and no-adapters regression
...
- Initial release
- 4 logical phases (3 execution steps): GATHER → ANALYZE+SYNTHESIZE → PRESENT
- Clio analyst agent (opus) reads all story artifacts inline for cross-story pattern recognition
- Artifact types consumed: progress JSON, review JSON, narrative logs, summaries, themis triage, mnemosyne reflections, pygmalion forging, hermes reports, hardening reports
- Pattern detection: recurring review findings, build efficiency, knowledge gaps, specialist reuse
- Previous retro continuity: action item follow-through scoring (COMPLETED/PARTIALLY_ADDRESSED/NOT_ADDRESSED/RECURRED)
- Next epic readiness assessment with blocker identification
- Playbook proposals with consolidation from per-story Mnemosyne reflections
- CLAUDE.md patches with very high bar (pointer-to-playbook pattern preferred, expect 0-1 per retro)
- Pantheon process suggestions as diffs (never auto-applied)
- Sprint status integration (marks `epic-X-retrospective: done`)
