...
**Claude Code** (default)
- Uses existing `_bmad/pantheon/` structure
- Full native support
...
In your project's `pantheon/config.yaml`:

> **Why `pantheon/`?** Module code lives in `_bmad/pantheon/` and gets wiped on
> reinstall (`rm -rf _bmad`). Config lives at the project root in `pantheon/config.yaml`
...
Artifacts saved to `{{sprint_artifacts}}/` (defaults to `_bmad-output/sprint-artifacts`):
```
completions/
