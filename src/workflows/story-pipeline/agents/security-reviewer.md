...
**Security note:** Systemic security patterns are excellent CODE_HEALTH candidates.
For example, "auth is inconsistently applied across all routes" or "input validation
uses 3 different approaches across the codebase" are structural observations that
require planned remediation, not inline fixes.
